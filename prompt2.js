export default async function prompt2(message, opts) {
  ...
  let input;
  if(type == "select") {
    input = `<select style="width:100%;height:100%;background:white;color:inherit;">${opts.options.map(o => `<option value="${o.value}">${o.content}</option>`).join("")}</select>`;
  } else if(type == "buttons") {
    input = opts.buttons.map(o => `<button style="margin-right:0.5rem;" data-value="${o.value}">${o.content}</button>`).join("");
  } else {
    input = `<input style="width:100%;height:100%;background:white;color:inherit;" type="${opts.type}">`;
  }
  ctn.innerHTML = `
    <div style="background:rgba(0,0,0,0.2); position:fixed; top:0; left:0; right:0; bottom:0; z-index:9999999; display:flex; justify-content:center; color:black;">
      <div style="width:400px; background:white; height: min-content; padding:1rem; border:1px solid #eaeaea; border-radius:3px; box-shadow: 0px 1px 10px 3px rgba(0,0,0,0.24); margin-top:0.5rem;">
        ...
        <div style="display:flex; height:2.3rem;">
          ...
          ${type !== "buttons" ? `<button>Submit</button>` : ""}
        </div>
      </div>
    </div>
  `;
